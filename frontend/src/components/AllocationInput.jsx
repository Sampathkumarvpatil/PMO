...
const AllocationInput = ({sidebarToggle}) => {
  ...
  const editAndClose = (index, name, role, allocationPercentage) => {
    ...
    editedAllocation.hrPerDay = allocationPercentage !== undefined ? calculateHrPerDay(parseInt(allocationPercentage)) : editedAllocation.hrPerDay;
  ...
  const calculateHrPerDay = (percentage) => {
    ...
    return percentage * 8 / 100; // Assuming 8 hours per day
  };
  const closeAll = () => {
    ...
    <div className="">
      ...
      <div className="col-span-3 p-2 border border-red-100 my-5 ">
        ...
        <div className="flex justify-center">
            <button className="flex justify-center items-center  mb-4 px-8 py-1.5 bg-blue-600 text-white font-bold rounded" onClick={addAllocationRow}>Add</button>
          </div>
        <TableContainer component={Paper} >
          ...
          >
            <TableHead className="bg-gray-300">
              <TableRow>
                <TableCell
                  style={{ borderRight: "1px solid #000", textAlign: "center" , width:'11%'}}
                >
                  Sl No.
                </TableCell>
                <TableCell
                  style={{ borderRight: "1px solid #000", textAlign: "center", width:'30%' }}
                >
                  Name
                </TableCell>
                <TableCell
                  style={{ borderRight: "1px solid #000", textAlign: "center", width:'29%' }}
                >
                  Role
                </TableCell>
                <TableCell
                  style={{ borderRight: "1px solid #000", textAlign: "center", width:'20%' }}
                >
                  Hr/Day
                </TableCell>
                <TableCell
            ...
            <TableBody className="border border-gray-200">
              {selectedSprintAllocations.map((allocation, index) => (
                <TableRow
                  key={index}
                ...
                >
                  ...
                  <TableCell style={{ textAlign: "center" }}>
                    <FaEllipsisH size={18} onClick={(mouseEvent) => showOptions(index, mouseEvent)}  style={{ cursor: "pointer" }} />
                    {optionsVisible === index && (
                      <OptionsBox
                      onEdit={() => onEditClick(index)}
                        deleteRow={() => deleteRow(index)}
                        position={{ top: optionsPosition.top, left: optionsPosition.left }}
                      />
                    )}
                  </TableCell>
                </TableRow>
      ...
      {editIndex !== null && (
        <EditPopup
          ...
          editedAllocationPercentage={allocations[editIndex].allocationPercentage}
          onSave={editAndClose}
          onClose={() => closeAll()}
        />
      )}
