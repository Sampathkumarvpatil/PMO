  ...
  return (
    <tr className="task-row">
      ...
      <td className='p-2 border-solid border-2 border-[#aaa]'>
        ...
        { totHours > 0 ? <>
          <div className='text-center font-black text-xl mx-auto'>{totHours} hrs</div>
          <h2 className='text-center'>resource:<strong className='text-green-700 font-black'>{res}</strong></h2>
          </> :
          <button onClick={()=>planningPokerHandler(item.id)}
      ...
      <td className='p-2 border-solid border-2 border-[#aaa]'>
        ...
        onClick={() => deleteTaskHandler(item.id)}>
          {/* delete icon */}
          <span className='text-lg w-full text-center flex justify-center'><MdDelete /></span>
