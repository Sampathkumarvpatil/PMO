...
const FileUpload = ({ sidebarToggle }) => {
  ...
  const handleUpload = () => {
    if (file) {
      ...
      reader.onload = (event) => {
        ...
        const newUploadedFiles = [...uploadedFiles, { name: file.name, content: fileContent, description: '' }];
  ...
  const updateStoredFiles = (files, descriptions) => {
    ...
    const storedFiles = JSON.parse(localStorage.getItem('uploadedFiles')) || {};
    storedFiles[`${selectedProjectName}${selectedSprintName}`] = files.map((file, index) => ({
      ...file,
  ...
  <thead>
    <tr>
      ...
      <th className="bg-gray-200 p-4 text-left text-center">Description</th>
      <th className="bg-gray-200 p-4 text-left text-center">Actions</th>
    </tr>
  </thead>
  <tbody>
    {uploadedFiles.map((uploadedFile, index) => (
      <tr key={index} className="bg-gray-100 border-b-2 border-gray-400">
        
        <td className="p-4 text-center">{`${index + 1}:`}</td>
        <td className="p-4 text-center">{` ${uploadedFile.name}`}</td>
        <td className="p-4 text-center">
          <textarea cols={40} rows={3} type="text" value={fileDescriptions[index]} onChange={(event) => handleDescriptionChange(index, event)} className="bg-gray-200 p-2 rounded-md mr-2" placeholder="Enter description" />
        </td>
        <td className="p-4 text-center">
          <button onClick={() => handleOpenFile(uploadedFile.content)} className="bg-blue-500 text-white px-4 py-2 rounded-md mr-2">Open</button>
          <button onClick={() => handleDeleteFile(index)} className="bg-red-500 text-white px-4 py-2 rounded-md">Delete</button>
        </td>
      </tr>
    ))}
  </tbody>
